...
async function main(): Promise<void> {
  const Logic: ContractFactory = await ethers.getContractFactory("SamuraiggNFT");
  const logic: Contract = await upgrades.deployProxy(Logic, [], { kind: "uups", initializer: "initialize" });
